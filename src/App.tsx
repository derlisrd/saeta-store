...
function DiscountBadge({ normal, promo }: { normal: number; promo: number }) {
...
function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <article className="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-slate-100 flex flex-col">
      <div className="relative bg-slate-50 aspect-square overflow-hidden">
        ...
        {hasDiscount && (
          <DiscountBadge normal={product.precio_normal} promo={product.precio_promocional!} />
        )}
        {product.category && (
      ...
      <div className="p-4 flex flex-col gap-2 flex-1">
        ...
        <button className="mt-2 w-full bg-slate-900 hover:bg-slate-700 active:scale-95 text-white text-sm font-medium py-2 px-4 rounded-xl transition-all duration-150">
          Ver producto
        </button>
      </div>
    </article>
